...
def run():
    ...
    try:
        ...
        with open(input_file_path, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(f):
                ...
                if not utils.is_valid_input(line_text):
                    ...
                    logging.warning(f'Skipping invalid input on line {i+1}: "{line_text[:100]}..."')
        ...
        run_timestamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
        ...
        logging.critical(f"A critical error occurred during the pipeline run: {e}", exc_info=True)
